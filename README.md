...
Every number in the manuscript is generated by `scripts/make_crossover_figures.py` from the
files in `eval_results/`; none is typed by hand. Re-running it reproduces the figures and
the macro file the manuscript cites, so each reported value is traceable to the cells here.
The manuscript itself is not included in this repository while it is under review.
